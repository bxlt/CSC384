...
class ExactInference(InferenceModule):
        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        for p in self.legalPositions:
            ...
            if emissionModel[trueDistance] > 0:
                currB = self.beliefs[p]
                allPossible[p] += emissionModel[trueDistance]*currB
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        # find all avaliable positions
        avaliables = util.Counter()
        Ps = self.legalPositions
        i = 0
        while (i< len(Ps)):
            # find the new distribution under the new position
            curr=Ps[i]
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, curr))
            keys = list(newPosDist.keys())
            j = 0
            # add all probability
            while (j< len(keys)):
                newProb = self.beliefs[curr]*newPosDist[keys[j]]
                avaliables[keys[j]] += newProb
                j+=1
            i+=1

        avaliables.normalize()
        self.beliefs = avaliables
        "*** END YOUR CODE HERE ***"
